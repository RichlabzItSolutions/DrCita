...
import com.drcita.user.adapter.MedicalMemberAdapter;
...
public class MedicalMembersListActivity extends LanguageBaseActivity {
    ActivityAddMemberBinding addMemberBinding;
    MedicalMemberAdapter membersAdapter;
        ...
        try{
            ...
            try {
                if (Constants.haveInternet(getApplicationContext())) {
                    ...
                    ApiClient.getRestAPI().getFetchAll(request).enqueue(new Callback<MemberResponse>() {
                        @Override
                        ...
                        public void onFailure(@NonNull Call<MemberResponse> call, @NonNull Throwable t) {
                            t.printStackTrace();
                            Toast.makeText(MedicalMembersListActivity.this, t.getMessage(), LENGTH_SHORT).show();
                            dismissLoadingDialog();
                        }
                    });
                }
    ...
    private void getMembersResponse(MemberResponse body) {
       ...
       membersAdapter=new MedicalMemberAdapter(MedicalMembersListActivity.this,body.getData());
    ...
    private void getRelationships() {
        try {
            ...
            if (Constants.haveInternet(getApplicationContext())) {
                ...
                Constants.InternetSettings(MedicalMembersListActivity.this);
            }
...
// 4. Prepare diseases list (can be empty)
                ...
                ApiClient.getRestAPI().addMemberNew(request).enqueue(new Callback<RestResponse>() {
                    public void onResponse(@NonNull Call<RestResponse> call, @NonNull Response<RestResponse> response) {
                        dismissLoadingDialog();
                        if (response.isSuccessful()) {
                            Toast.makeText(MedicalMembersListActivity.this, response.body().getMessage(),Toast.LENGTH_SHORT).show();
                             setupMembers();
